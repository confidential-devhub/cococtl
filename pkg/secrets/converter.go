...
// kbsNamespace is used in the KBS URI (e.g. "default" for kbs:///default/secretName/key).
// k8sNamespace is the namespace where the K8s sealed secret resource is created (from the input manifest).
func ConvertToSealed(kbsNamespace, k8sNamespace, secretName, key string) (*SealedSecretData, error) {
	// Strip leading "." from key name as KBS doesn't support it in URIs
	// e.g., ".dockerconfigjson" becomes "dockerconfigjson"
	cleanKey := strings.TrimPrefix(key, ".")

	// Build KBS resource URI (always uses kbsNamespace, typically "default")
	// Format: kbs:///namespace/secretName/key
	resourceURI := fmt.Sprintf("kbs:///%s/%s/%s", kbsNamespace, secretName, cleanKey)
	...
	return &SealedSecretData{
		...
		Key:          cleanKey,
		Namespace:    k8sNamespace, // namespace from input manifest for K8s resource
	}, nil
}
...
func ConvertSecrets(refs []SecretReference, inspectedKeys map[string]*SecretKeys) ([]*SealedSecretData, error) {
	...
	const kbsNamespace = "default" // KBS URI always uses "default"; K8s secret uses manifest namespace

	for _, ref := range refs {
		// Determine which keys to convert (from cluster inspection or ref)
		...
		// K8s sealed secret namespace: from input file (ref.Namespace from manifest or current context)
		k8sNamespace := ref.Namespace
		if k8sNamespace == "" {
			k8sNamespace = "default"
		}

		// Convert each key to sealed secret (KBS URI uses "default"; K8s resource uses manifest namespace)
		for _, key := range keysToConvert {
			sealed, err := ConvertToSealed(kbsNamespace, k8sNamespace, ref.Name, key)
			if err != nil {
				return nil, err
			}
